import numpy as np
from nonlinear_benchmarks.nanodrone_utils import quat_mul, quat_conj
...
# ============================================================
def compute_simerr(metric_dict, max_horizon=50):
    sim_p = sum(metric_dict["pos"][h] for h in range(1, max_horizon + 1))
    sim_v = sum(metric_dict["vel"][h] for h in range(1, max_horizon + 1))
    sim_R = sum(metric_dict["rot"][h] for h in range(1, max_horizon + 1))
    sim_w = sum(metric_dict["omega"][h] for h in range(1, max_horizon + 1))
